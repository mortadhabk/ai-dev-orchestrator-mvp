...
async function main() {
    ...
    const trello = new TrelloClient(process.env.TRELLO_BASE_URL ?? 'https://api.trello.com/1', process.env.TRELLO_KEY, process.env.TRELLO_TOKEN);
    const github = new GithubClient(process.env.GITHUB_API_URL ?? 'https://api.github.com', process.env.GITHUB_TOKEN, process.env.GITHUB_OWNER, process.env.GITHUB_REPO);
    const card = await trello.getCard(process.env.CARD_ID);
    ...
    const shortCardId = process.env.CARD_ID.slice(0, 8);
    ...
    await runGit(['commit', '-m', `chore(${process.env.AGENT_NAME}): work on ${shortCardId}`], repoDir);
    await runGit(['push', 'origin', process.env.BRANCH_NAME], repoDir);
