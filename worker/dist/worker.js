...
const trello = new TrelloReporter(process.env.TRELLO_BASE_URL ?? 'https://api.trello.com/1', process.env.TRELLO_KEY, process.env.TRELLO_TOKEN);
const worker = new Worker('agent-jobs', async (job) => {
    ...
    const result = await runAgentContainer({
        ...
        env: {
            ...
            TRELLO_LIST_REVIEW_ID: process.env.TRELLO_LIST_REVIEW_ID,
        },
    });
    ...
    await trello.move(data.cardId, process.env.TRELLO_LIST_REVIEW_ID);
    return { prUrl };
