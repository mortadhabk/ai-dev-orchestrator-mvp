...
const AGENTS = ['ai-backend', 'ai-frontend', 'ai-devops'];
function detectAgent(commentText) {
    const lowered = commentText.toLowerCase();
    return AGENTS.find((agent) => lowered.includes(`@${agent}`)) ?? null;
}
function getShortCardId(cardId) {
...
export function trelloWebhookRouter(deps) {
    ...
    router.post('/trello', async (req, res) => {
        try {
            ...
            await deps.trello.addComment(cardId, `✅ Job démarré par ${agent}. Branch: ${branch}`);
